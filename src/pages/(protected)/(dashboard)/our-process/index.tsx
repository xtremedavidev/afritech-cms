import { useGetProcesses } from "@/src/commons/api/our-process"
...
import ProcessesTable from "./_component/ProcessesTable.component"
...
const OurProcessPage = ({ className }: Props) => {
		...
		<div className={twMerge("flex w-full flex-col gap-y-4 px-4 pb-[5%]", className)}>
			<div className="mt-4 flex w-full flex-row items-center justify-between">
				<div className="flex flex-col">
					<p className="font-heading text-2xl font-bold">Our Process</p>
					<p className={"text-sm"}>Compose and publish the process section on your website.</p>
				</div>
				<div className={"flex w-[60%] items-end justify-end gap-x-4 "}>
					<CustomButton
						className={""}
						startIcon={<PenLine />}
						text={"Create Process Item"}
						variant={"primary"}
						onClick={() => router.push("/our-process/create")}
					/>
				</div>
			</div>
			...
			{!!searchQuery?.length ? <SearchedProcesses {...sharedProps} /> : <AllProcesses />}
		</div>
	)
}
...
const AllProcesses = ({ startDate, endDate, limit, className }: Props) => {
	const [page, setPage] = React.useState(1)

	const { data = [], isPending } = useGetProcesses({ page, limit })
	return (
		<div className={""}>
			{isPending ? <Loading.Spinner /> : <ProcessesTable data={data} />}
...
const SearchedProcesses = ({ startDate, endDate, limit, search, className }: Props) => {
	const [page, setPage] = React.useState(1)

	const { data = [], isPending } = useGetProcesses({ page, limit, search }, { enabled: search?.length > 2 })
	return (
		<>
			{isPending ? (
