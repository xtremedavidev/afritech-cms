...
export default function ProjectsHeroEditor({ title, subtitle, initialData }: Props) {
	const { loader } = useGlobalStore()
	const { mutateAsync: updateHero, isPending: isUpdating } = useUpdateHero("project")

	const onSubmit = ({ data }: { data: Partial<IHomeHero> }) => {
		loader.start()
		updateHero(data)
			.then((res) => toast.success("Projects hero section updated!"))
