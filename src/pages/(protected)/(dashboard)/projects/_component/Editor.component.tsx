...
export default function ProjectEditor({
	...
	const form = useForm(
		CONFIG.useForm({
			schema: (z) => ({
				...
				tools:
					viewType !== "create"
						? z.array(z.string()).min(1, "At least one tag is required").max(8)
	...
	useEffect(() => {
		...
		form.setValue("projectUrl", initialData.projectUrl || "")
		...
		return (
			<>
				...
				<FormInput
					className="flex-1"
					type={"url"}
					errors={form.formState?.errors?.projectUrl}
					register={form.register("projectUrl")}
					label={"Project Link"}
