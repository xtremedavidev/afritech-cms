...
import { useGetTeam } from "src/commons/api/teams"
...
import TeamTable from "@src/pages/(protected)/(dashboard)/team/_component/TeamTable.component"
...
const TeamPage = ({ className }: Props) => {
		...
		<div className={twMerge("flex w-full flex-col gap-y-4 px-4 pb-[5%]", className)}>
			<div className="mt-4 flex w-full flex-row items-center justify-between">
				<div className="flex flex-col">
					<p className="font-heading text-2xl font-bold">Team</p>
					<p className={"text-sm"}>Add and edit your team members profile.</p>
				</div>
			...
			{!!searchQuery?.length ? <SearchedTeam {...sharedProps} /> : <AllTeam />}
		</div>
	)
}
...
const AllTeam = ({ startDate, endDate, limit, className }: Props) => {
	const [page, setPage] = React.useState(1)

	const { data = [], isPending } = useGetTeam({ page, limit })

	return (
		<div className={""}>
			{isPending ? <Loading.Spinner /> : <TeamTable data={data} />}
...
const SearchedTeam = ({ startDate, endDate, limit, search, className }: Props) => {
	const [page, setPage] = React.useState(1)

	const { data = [], isPending } = useGetTeam({ page, limit, search }, { enabled: search?.length > 2 })
	return (
		<>
			{isPending ? (
