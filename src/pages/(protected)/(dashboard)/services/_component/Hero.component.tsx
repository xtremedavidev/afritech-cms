...
export default function ServicesHeroEditor({ title, subtitle, initialData }: Props) {
	const { loader } = useGlobalStore()
	const { mutateAsync: updateHero, isPending: isUpdating } = useUpdateHero("service")

	const onSubmit = ({ data }: { data: Partial<IHomeHero> }) => {
		loader.start()
		updateHero(data)
			.then((res) => toast.success("Services hero section updated!"))
