...
import ServicesTable from "@src/pages/(protected)/(dashboard)/services/_component/ServicesTable.component"
import { Search01Icon, Search02Icon } from "hugeicons-react"
import { PenLine } from "lucide-react"
import moment from "moment"
import React, { useState } from "react"
import { useSearchParams } from "react-router-dom"
import { useGetServices } from "src/commons/api/services"
import { twMerge } from "tailwind-merge"
import ServicesHeroEditor from "./_component/Hero.component"
...
const ServicesPage = ({ className }: Props) => {
	const router = useRouter()
	const [params, setParams] = useSearchParams()

	const { data } = useGetHero("service")
		...
		<div className={twMerge("flex w-full flex-col gap-y-4 px-4 pb-[5%]", className)}>
			<div className="mt-4 flex w-full flex-row items-center justify-between">
				<div className="flex flex-col">
					<p className="font-heading text-2xl font-bold">Services</p>
					<p className={"text-sm"}>Compose and publish services to your website.</p>
				</div>
			...
			<ServicesHeroEditor
				title="Hero section"
				subtitle="Edit the services hero section on your website."
				initialData={data}
			/>

			<div className="mt-[5%] gap-y-3">
				<h2 className="text-2xl font-medium">Services list</h2>
			...
			{!!searchQuery?.length ? <SearchedServices {...sharedProps} /> : <AllServices />}
		</div>
	)
}
...
const AllServices = ({ startDate, endDate, limit, className }: Props) => {
	const [page, setPage] = React.useState(1)

	const { data = [], isPending } = useGetServices({ page, limit })

	return (
		<div className={""}>
			{isPending ? <Loading.Spinner /> : <ServicesTable data={data} />}
...
const SearchedServices = ({ startDate, endDate, limit, search, className }: Props) => {
	const [page, setPage] = React.useState(1)

	const { data = [], isPending } = useGetServices({ page, limit, search }, { enabled: search?.length > 2 })
	return (
		<>
			{isPending ? (
