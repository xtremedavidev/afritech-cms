import { IAboutContent } from "@/src/commons/api/about-us/index.interface"
...
interface Props {
	title: string
	subtitle?: string
	initialData?: IAboutContent
	onSubmit: (options: { data: Partial<IAboutContent> }) => void
	isLoading?: boolean
}

export default function AboutHistoryEditor({ title, subtitle, initialData, onSubmit, isLoading = false }: Props) {
	const form = useForm(
		CONFIG.useForm({
			schema: (z) => ({
	...
	useEffect(() => {
		...
		form.setValue("aboutDescription", initialData.aboutDescription || "")
	}, [initialData])
	...
	const renderSidebarFields = () => {
		return (
			<>
				...
				<FormTextArea
					className="flex-1"
					errors={form.formState?.errors?.aboutDescription}
					register={form.register("aboutDescription")}
