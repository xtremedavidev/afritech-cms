import { useGetDeliverables, useGetDeliverablesHero } from "@/src/commons/api/our-deliverables"
...
import DeliverablesTable from "./_component/DeliverablesTable.component"
import DeliverablesTitleEditor from "./_component/Title.component"
...
const DeliverablesPage = ({ className }: Props) => {
	const router = useRouter()
	const [params, setParams] = useSearchParams()

	const { data: title } = useGetDeliverablesHero()
		...
		<div className={twMerge("flex w-full flex-col gap-y-4 px-4 pb-[5%]", className)}>
			<div className="mt-4 flex w-full flex-row items-center justify-between">
				<div className="flex flex-col">
					<p className="font-heading text-2xl font-bold">Deliverables</p>
					<p className={"text-sm"}>Edit the contents of your website's deliverables section.</p>
				</div>
				<div className={"flex w-[60%] items-end justify-end gap-x-4 "}>
					<CustomButton
						...
						variant={"primary"}
						onClick={() => router.push("/our-deliverables/create")}
					/>
				</div>
			</div>

			<DeliverablesTitleEditor
				title="Title section"
				subtitle="Edit the title and description of the deliverables section on your website."
				initialData={title}
			/>

			<div className="mt-[5%] space-y-3">
				<h2 className="text-2xl font-medium">Deliverables list</h2>
			...
			{!!searchQuery?.length ? <SearchedDeliverables {...sharedProps} /> : <AllDeliverables />}
		</div>
	)
}
...
const AllDeliverables = ({ startDate, endDate, limit, className }: Props) => {
	const [page, setPage] = React.useState(1)

	const { data = [], isPending } = useGetDeliverables({ page, limit })
	return (
		<div className={""}>
			{isPending ? <Loading.Spinner /> : <DeliverablesTable data={data} />}
...
const SearchedDeliverables = ({ startDate, endDate, limit, search, className }: Props) => {
	const [page, setPage] = React.useState(1)

	const { data = [], isPending } = useGetDeliverables({ page, limit, search }, { enabled: search?.length > 2 })
	return (
		<>
			{isPending ? (
