...
export default function DeliverableEditor({
	...
	const form = useForm(
		CONFIG.useForm({
			schema: (z) => ({
				...
				description: z.string().min(1, "Metric value is required"),
	...
	const handleSubmit = form.handleSubmit(
		(data) => {
			onSubmit({ data: { ...data, status: viewType === "create" ? "published" : data.status } })
		},
		(errors) => {
	...
	const handleSaveDraft = form.handleSubmit(
		(data) => {
			onSubmit({ data: { ...data, status: "draft" } })
		},
		(errors) => {
			Object.entries(errors).forEach(([key, value]) => {
				if (value?.message) {
		...
		<div className={twMerge("flex w-full flex-col gap-y-4 px-[5%] pb-[5%]")}>
			...
			<div className="flex flex-col">
				...
				<FormInput
					...
					label={"Order"}
					placeholder={"Enter the order of this deliverable"}
					disabled={viewType === "view"}
				/>
			</div>
		</div>
