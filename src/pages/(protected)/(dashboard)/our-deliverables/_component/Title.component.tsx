import { useUpdateDeliverablesHero } from "@/src/commons/api/our-deliverables"
import { IDeliverableHero } from "@/src/commons/api/our-deliverables/index.interface"
...
export default function DeliverablesTitleEditor({ title, subtitle, initialData }: Props) {
	const { loader } = useGlobalStore()
	const { mutateAsync: updateHero, isPending: isUpdating } = useUpdateDeliverablesHero()

	const onSubmit = ({ data }: { data: Partial<IDeliverableHero> }) => {
		loader.start()
		updateHero(data)
			.then((res) => toast.success("Deliverable section updated!"))
	...
	const renderSidebarFields = () => {
		return (
			<>
				...
				<FormTextArea
					className="flex-1"
					errors={form.formState?.errors?.subtitle}
